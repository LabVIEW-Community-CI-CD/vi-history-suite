...
- **Context key names and `when` clause**: the request specifies a `when` clause like  
  ...
  The repo currently uses `viHistorySuite.eligiblePaths` and `viHistorySuite.isWorkspaceTrusted` (custom). VS Code already exposes `isWorkspaceTrusted` for `when` clauses and provides the Workspace Trust API; prefer using the built‑in key for compliance and simplicity. citeturn25view1turn24view4
- **Editor context menus**: the repo contributes only to `explorer/context`. The request asks to cover `editor/title/context` (optional) and mentions `editor/title` and `editor/context` locations. VS Code supports these in `contributes.menus`. citeturn29view2turn29view3
- **Report generation**: no implementation exists for generating LVCompare/LabVIEWCLI reports; must be added, including:
  ...
  - Storage strategy (`context.storageUri` recommended) and secure webview linking (via `asWebviewUri` and `localResourceRoots`). citeturn11view0turn19view1turn19view2
- **LabVIEW 2026 Q1 32/64 adaptation**: repo has settings placeholders but no robust runtime detection. Must implement:
  ...
  - Cross‑platform fallbacks (macOS, Linux) acknowledging current NI platform constraints (e.g., macOS Community Edition notes and differing support timelines). citeturn7search9turn7search3turn7search10
- **Progress UX**: notification progress exists; request also wants a **progress bar design** with percent/items/ETA and surfaced through:
  ...
  - optional webview progress messaging. citeturn15view0turn11view0turn12view0turn14view1

## VS Code extension APIs and UX primitives to use

### Menus, commands, when clauses, and setContext membership maps

**Menu contributions** are declared in `package.json` under `contributes.menus`. VS Code supports menu locations including `explorer/context`, `editor/context`, `editor/title`, and `editor/title/context`. citeturn29view2turn29view1

A critical enabling pattern is to compute eligibility at runtime and maintain a context key (via the built‑in `setContext` command) whose value is an object used with the **`in` operator** in a menu `when` clause. VS Code explicitly documents `in` / `not in` plus `setContext` for dynamic menu gating. citeturn24view1turn32view1

Key details:
- `resourcePath` is a context key that represents the active resource path and can be used for matching. citeturn24view3
- VS Code includes a `gitOpenRepositoryCount` context key; you can use comparisons like `gitOpenRepositoryCount >= 1`. citeturn24view4
- Workspace Trust can be used in UI gating using the built‑in `isWorkspaceTrusted` context key. citeturn25view1
- Commands invoked from a context menu are passed the selected resource URI when VS Code can infer it (Explorer passes URI of selected resource; editor passes URI of document). citeturn30search0

**Enablement vs visibility**: VS Code notes that `when` applies to menus while `enablement` applies to commands broadly. If you want “visible only if eligible,” prefer `when` gating the menu item; also keep command handler resilient (show a message if invoked without eligibility). citeturn29view4turn25view0
...
- `onStartupFinished` triggers after VS Code startup and is designed not to slow startup. citeturn28view1
- `workspaceContains:<glob>` activates when an opened folder contains at least one file matching a glob pattern. citeturn28view2

Caveat: `workspaceContains` is based on matching a **file** glob; relying on `.git/` (a directory) may be unreliable. This is why many extensions use `onStartupFinished` plus runtime checks (e.g., `gitOpenRepositoryCount >= 1` in menus) rather than trying to “pre‑detect git repositories” via `workspaceContains`. citeturn28view2turn24view4
...
- Use `Webview.asWebviewUri` to load local resources securely. citeturn19view2
- Restrict file access via `WebviewOptions.localResourceRoots` (least privilege). citeturn19view1
- Follow webview security practices: minimal capabilities (`enableScripts` only when needed) and a CSP. citeturn19view3

For storing generated HTML comparison reports, use `ExtensionContext.storageUri` (workspace‑specific directory) which is recommended for larger workspace‑scoped files. citeturn11view0turn3search3
...
As of the referenced sources, **TimelineProvider is still a proposed API** (`vscode.proposed.timeline.d.ts` exists), meaning it is **not publishable** without enabling proposed APIs (Insiders + `--enable-proposed-api`), and proposed APIs cannot be used in published Marketplace extensions. citeturn17search0turn16search1turn16search14
...
- `workspace.fs.readFile(uri)` reads the **entire contents** of a file as bytes; there is no range API, so it’s inefficient for probing just 12 bytes. citeturn23view0
...
Additionally, Workspace Trust should gate any tool execution (Git CLI, LVCompare, LabVIEWCLI) because these execute external binaries. VS Code provides static declarations via `capabilities.untrustedWorkspaces` and a runtime API `workspace.isTrusted`. citeturn25view0turn25view3
...
1) Attempt to use **`vscode.git` API** for repo discovery, blob reads (`Repository.buffer`) and `toGitUri` diff/open. citeturn27view1turn27view3  
2) Use **Git CLI** as a reliable fallback (especially for `--follow` semantics, which may not be identical in all APIs). citeturn4search1turn5search2

### Required Git commands and why

Tracked-file enumeration should use:

- `git ls-files -z` (tracked files; NUL‑terminated, no quoting), ideal for safe parsing of unusual filenames. citeturn5search2
...
- `git log -n 2 --format=%H --follow -- <path>`  
  ...
  - `--follow` continues history across renames, but Git documents that it works only for a **single file**. citeturn4search1turn5search2

Blob verification and report generation inputs should use:

- `git show <hash>:<path>` to retrieve the blob contents: Git’s `git show` documentation states that for plain blobs, it shows the plain contents. citeturn5search0turn5search11  
  Combine with `gitrevisions` syntax (`<rev>:<path>`) rules if you need to support richer revision specs. citeturn5search1

Repo root and HEAD hash:

- `git rev-parse --show-toplevel` to anchor paths and determine repo root. citeturn4search3  
- `git rev-parse HEAD` to cache eligibility per HEAD. (Git plumbing command; same man page.) citeturn4search3
...
1) Determine active repositories:
   - Use `vscode.git` API `repositories` list when available. citeturn27view1turn26search0  
   - Fallback: attempt `git rev-parse --show-toplevel` in each workspace folder.

2) For each repository:
   - Enumerate tracked files: `git ls-files -z`. citeturn5search2  
   ...
   - If magic‑byte passes, run bounded commit query:
     ...
     - Eligible iff number of hashes returned ≥ 2. citeturn4search1turn5search2

3) Cache and invalidate:
   - Cache key: `(repoRoot, relPath, HEAD)`; if HEAD unchanged, reuse previous eligibility result.
   - Invalidate on:
     - `vscode.git` repository events when available (`onDidOpenRepository`, `onDidCloseRepository`, and ideally repository state change). citeturn27view1turn27view5
...
5) Best-effort rename handling:
   - `--follow` is best‑effort and documented as single-file only. citeturn4search1  
...
- A resource header begins with `RSRC` and a version; then a “file type” field such as `LVIN` (VI), `LVCC` (class), or related values appears in early bytes. citeturn10search7turn9search1turn10search19  
- NI Community discussion about VI “magic number” references `LVINLBVW` and notes structure details immediately before it. citeturn9search5  
- PRONOM research issue for LabVIEW VIs aligns with these signatures in its format identification effort. citeturn10search0
...
Optionally, to reduce false positives, a “strict” mode may also validate `RSRC` at offset 0. Ryan Pacini’s file format write‑up describes RSRC header bytes at the start of VI files. citeturn10search7turn9search0

### Reading strategy: Node partial reads first, workspace.fs fallback

Because `workspace.fs.readFile` reads the whole file, it’s potentially expensive for large repos or large binary files. citeturn23view0
...
export async function isLabviewViByMagic(uri: vscode.Uri): Promise<boolean> {
  ...
  try {
    const bytes = await vscode.workspace.fs.readFile(uri); // whole file citeturn23view0
...
Rationale:
- `resourcePath in <contextKey>` uses VS Code’s `in` operator + membership map pattern. citeturn24view1turn24view3
- Workspace trust uses the built‑in `isWorkspaceTrusted` key. citeturn25view1
- Git repo presence uses `gitOpenRepositoryCount >= 1`. citeturn24view4
- `editor/title/context` is supported as a menu location. citeturn29view2turn29view3
- VS Code recommends showing actions only when contextually appropriate; this aligns with the UX guidelines for context menus. citeturn13search3turn30search11
...
- Row actions:
  - **Open @ commit**: `vscode.open` on `git.toGitUri(fileUri, hash)` or CLI fallback. citeturn27view1turn32view1
  - **Diff vs previous**: call `vscode.diff(leftUri, rightUri, title)` citeturn32view1
...
- Convert report file URIs using `asWebviewUri` and link them into the HTML. citeturn19view1turn19view2turn11view0
- Add CSP and minimal scripts. citeturn19view3
...
VS Code’s progress API supports reporting progress increments/messages, and includes a sample demonstrating cancellable notifications and `progress.report({ increment, message })`. citeturn15view0turn11view0
...
VS Code UX guidelines recommend using a status bar item with a loading icon for discreet background progress. citeturn12view0turn14view1
...
The icon reference documents spinning animation via `~spin` for `sync`, `loading`, and `gear`. citeturn14view1
...
NI documents a LabVIEW CLI operation **CreateComparisonReport** that compares two VIs and outputs an HTML/XML/Word/text report; it explicitly notes it can compare VIs “without including VI dependencies,” reducing dependency-related compare errors. citeturn6search0turn6search8

Additionally, NI documents command-line usage of LVCompare/LVMerge for source control integration and that LVCompare relates to the “Compare VIs” UI. citeturn7search4turn8search0turn6search2

Important caveat: NI notes that comparing VIs with the same name is not supported by LVCompare.exe; a source control provider typically handles temporary renaming. citeturn8search8turn8search5  
...
1) **Git API**: use `Repository.buffer(ref, path)` to fetch a `Buffer` of blob contents (fast, no process spawn). This method exists in the Git extension API. citeturn27view3  
2) **Git CLI**: `git show <hash>:<path>` and read the first 12 bytes from stdout.

Git show for blobs returns the plain contents. citeturn5search0
...
This uses VS Code’s recommended storage mechanism for large files. citeturn11view0turn3search3

In the webview:
- Add the report output directory to `localResourceRoots`. citeturn19view1
- Convert report file URI with `asWebviewUri` and provide a clickable link. citeturn19view2
- Keep `localResourceRoots` as narrow as possible and add a CSP. citeturn19view3
...
Prefer the official Git extension API pattern documented in the Git extension README, and declare `"extensionDependencies": ["vscode.git"]` to ensure activation order. citeturn26search0turn27view1

```ts
import * as vscode from 'vscode';

import type { API as GitApi, GitExtension } from './git-api/git'; // copy from vscode.git repo (see README) citeturn26search0

async function getGitApi(): Promise<GitApi | undefined> {
  const ext = vscode.extensions.getExtension<GitExtension>('vscode.git');
  if (!ext) return undefined;
  const gitExt = ext.isActive ? ext.exports : await ext.activate();
  // getAPI(1) may throw if git extension is disabled citeturn27view4
...
Use `git ls-files -z` for tracked files and safe parsing. citeturn5search2

Use `git log --follow` noting it works only for a single file. citeturn4search1
...
async function listTrackedFiles(repoRoot: string): Promise<string[]> {
  const { stdout } = await runGit(['ls-files', '-z'], repoRoot); // citeturn5search2
  return stdout
    .toString('utf8')
    .split('\0')
    .filter(Boolean);
...
async function hasAtLeastTwoModifyingCommits(repoRoot: string, relPath: string): Promise<boolean> {
  const { stdout } = await runGit(
    ['log', '-n', '2', '--format=%H', '--follow', '--', relPath], // citeturn4search1turn5search2
    repoRoot
  );
...
VS Code UX guidance recommends a discreet progress status bar item for background tasks. citeturn12view0turn14view1
...
export function createIndexerStatusBar(): vscode.StatusBarItem {
  ...
  item.text = '$(loading~spin) VI History: indexing…'; // spinner supported citeturn14view1
...
`vscode.diff` is a built‑in command. citeturn32view1  
`toGitUri` is part of the Git extension API. citeturn27view1
...
export async function openDiff(git: GitApi, fileUri: vscode.Uri, leftHash: string, rightHash: string) {
  const left = git.toGitUri(fileUri, leftHash);   // citeturn27view1
  const right = git.toGitUri(fileUri, rightHash);
  await vscode.commands.executeCommand('vscode.diff', left, right, `VI Diff: ${leftHash.slice(0,8)} ↔ ${rightHash.slice(0,8)}`); // citeturn32view1
}
```

### Webview creation with localResourceRoots for report linking

Use `localResourceRoots` and `asWebviewUri`. citeturn19view1turn19view2
...
</html>`; // CSP guidance citeturn19view3
...
The official guide describes these options and recommends using `workspace.isTrusted` + `onDidGrantWorkspaceTrust`, plus the `isWorkspaceTrusted` context key for UI gating. citeturn25view0turn25view3turn25view1

The extension should declare:
- `extensionDependencies: ["vscode.git"]` so the Git API is ready when needed. citeturn26search0
- `activationEvents`: prefer `onStartupFinished` (to allow indexing without startup penalty) and `onCommand:labviewViHistory.open`. citeturn28view1turn28view3  
...
- Install: `npm install -g @vscode/vsce` citeturn31view1
- Package: `vsce package` (produces `.vsix`) citeturn31view2
- Publish: `vsce publish` (requires Marketplace publisher + Azure DevOps PAT with Marketplace Manage scope) citeturn31view3turn31view1
- Install locally for testing: `code --install-extension my-extension-0.0.1.vsix` citeturn31view2

Bundling considerations:
- Only bundled extensions can be used in VS Code for Web; if your extension uses `child_process` (Git CLI, LVCompare), it is inherently desktop/remote‑host oriented. citeturn18search1turn2search3

Testing:
- Use VS Code’s official “Testing Extensions” guide with `@vscode/test-cli` and `@vscode/test-electron`. citeturn1search3
- CI can use the official Continuous Integration guidance. citeturn18search0
...
- **Magic-byte detection**
  ...
  - File with correct magic but wrong/absent `RSRC` header → behavior depends on “strict” mode (explicitly configurable). citeturn10search7turn9search1

- **Git eligibility**
  - Tracked VI with 0 or 1 modifying commits → menu hidden.
  - Tracked VI with ≥2 commits → menu shown.
  - Untracked file, even if it is a VI by content → menu hidden (since `git ls-files` enumerates tracked files). citeturn5search2
  - Renamed file:
    - With simple rename → `--follow` should find prior commits; confirm best-effort. Git documents `--follow` works only for a single file; document limitations. citeturn4search1
...
- **Remote/virtual workspaces**
  ...
  - In virtual workspaces / vscode.dev: child_process not available; the extension should either disable itself or degrade to Git API capabilities only (but comparing blobs and running LVCompare is not feasible). Bundling docs emphasize web environment constraints. citeturn2search3turn18search1

- **Missing git binary / disabled Git extension**
  - VS Code’s docs note Git must be installed for GitHub workflows; handle “git not found” with a clear message, and/or rely on `vscode.git` and report if it’s disabled. citeturn26search10turn27view4

- **LVCompare same-name limitation**
  - Validate that LVCompare fails if both paths have the same filename and adjust by writing blobs to unique temp names. NI notes same-name compare isn’t supported by LVCompare.exe. citeturn8search8turn8search5
...
- Default LabVIEW install locations differ by bitness:
  ...
  - macOS: `/Applications/National Instruments/LabVIEW <VERSION> 64-bit` citeturn7search9

- LabVIEW registry presence is documented (version stored under `HKLM\Software\National Instruments\LabVIEW`, with keys like Type/Version). citeturn7search6turn7search2

- A community example explicitly details listing installed LabVIEW environments including bitness and use of WOW6432Node separation. citeturn7search10

- LVCompare source control configuration is documented and notes availability only in LabVIEW Professional Development System. citeturn8search0turn8search2

- LVMerge docs show the first argument can optionally be a path to `LabVIEW.exe` to select the LabVIEW version used for merge operations. citeturn6search2  
...
2) **Auto-discovery (Windows)**
   - Probe common LVCompare paths (examples commonly used in NI guidance and industry how-tos include `C:\Program Files (x86)\National Instruments\Shared\LabVIEW Compare\LVCompare.exe`, and sometimes non‑x86 depending on installation). See NI integration guidance that explicitly references the x86 path for 32-bit. citeturn6search13turn6search1  
   - Discover installed LabVIEW versions and bitness:
     - Query registry 64-bit view: `HKLM\SOFTWARE\National Instruments\LabVIEW\...`
     - Query registry 32-bit view: `HKLM\SOFTWARE\WOW6432Node\National Instruments\LabVIEW\...` (community example). citeturn7search10turn7search6  
   - If registry probing is not feasible, scan default install directories for `LabVIEW.exe` using the documented defaults. citeturn7search9

3) **Auto-discovery (macOS/Linux)**
   - Scan documented default install locations. citeturn7search9
   - Note macOS availability constraints: NI indicates LabVIEW Professional for macOS ended at 2023 Q3 and later availability is Community Edition beginning with 2025 Q3. This impacts LVCompare availability and should be documented. citeturn7search3
...
4) **Both architectures installed**
   - If the “currently running LabVIEW instance” is detectable (unspecified): optional advanced feature to inspect running processes and match LabVIEW.exe path. If not implemented, prefer:
     ...
     - x86 by default (NI generally recommends 32-bit for compatibility reasons in general guidance). citeturn6search3
...
// Simplified: scan default install roots (registry probing recommended for robustness) citeturn7search9
...
- `package.json` updates:
  ...
  - `capabilities.untrustedWorkspaces` set to `limited` with correct description and restrictedConfigurations if needed citeturn25view3turn24view1  
...
- Improve progress reporting:
  - percent/items/ETA in `withProgress` (increment/message) citeturn15view0turn11view0
  - status bar discreet progress item per UX guidelines citeturn12view0turn14view1
...
- Add `localResourceRoots` and CSP hardened HTML. citeturn19view1turn19view3  
...
- Report service:
  ...
  - Verify both blobs are VI via magic bytes (either `Repository.buffer(ref,path)` citeturn27view3 or `git show`) citeturn5search0
  - Generate HTML report using:
    - LabVIEWCLI CreateComparisonReport citeturn6search0turn6search8 and/or LVCompare
  - Write report to `context.storageUri` with name `{type}-report-{fullFilename}.html`. citeturn11view0
- Webview links to generated report via `asWebviewUri`. citeturn19view2  
...
- Runtime detection:
  - Windows registry + install scanning heuristics citeturn7search10turn7search9turn7search6
  - macOS/Linux scanning with platform notes citeturn7search9turn7search3
- Handle “both installed” selection logic and user override. citeturn6search3  
...
- Integration tests using `@vscode/test-electron` citeturn1search3 and a small fixture repo.
- CI workflow using official guidance. citeturn18search0turn31view4  
Acceptance: tests run locally and in CI, covering edge cases.

**Milestone: Packaging and release (4–8 hours)**  
Deliverables:
- `vsce package` produces a VSIX. citeturn31view2
- Document installation, trust requirements, tool dependencies.
- Marketplace publishing steps (PAT, publisher, vsce publish). citeturn31view3turn31view1  
...
- The exact LVCompare command-line switches to produce HTML reports are not fully specified by NI docs in the cited sources; therefore, the report generation recommends **LabVIEWCLI CreateComparisonReport** as the most explicitly documented report generator. citeturn6search0turn6search8
...
- Web extension (browser) support is not a target; because the extension shells out to git/LVCompare and uses file IO, it should be treated as desktop/remote-host oriented. citeturn18search1turn2search3
...
- VS Code:
  - Contribution points / menus and command parameter inference citeturn29view1turn30search0
  - When clause contexts (`in`, `resourcePath`, `gitOpenRepositoryCount`) citeturn24view1turn24view3turn24view4
  - Webview API (`localResourceRoots`, `asWebviewUri`, CSP/security) citeturn19view1turn19view2turn19view3
  - Workspace trust: `capabilities.untrustedWorkspaces`, `workspace.isTrusted`, `isWorkspaceTrusted` citeturn25view3turn25view1
  - Progress API and sample citeturn11view0turn15view0
  - Status bar UX guidelines and spinner icons citeturn12view0turn14view1
  - Built-in commands (`vscode.diff`, `setContext`) citeturn32view1
  - Publishing extensions via vsce citeturn31view1turn31view2turn31view3
  - Testing extensions citeturn1search3
  - CI citeturn18search0
  - Git extension API README and `git.d.ts` (toGitUri, Repository.log, Repository.buffer) citeturn26search0turn27view1turn27view3turn27view2
  - Proposed API limitations (TimelineProvider) citeturn16search1turn17search0turn16search14

- Git:
  - `git ls-files -z` citeturn5search2
  - `git log --follow` limitations citeturn4search1
  - `git show` blob contents citeturn5search0
  - `git rev-parse` citeturn4search3
  - `gitrevisions` syntax citeturn5search1

- NI / LabVIEW:
  - LabVIEWCLI CreateComparisonReport citeturn6search0turn6search8
  - LVCompare / LVMerge source control configuration and command-line usage citeturn8search0turn6search2
  - LVCompare naming/same-name limitation note citeturn8search8turn8search5
  - Default install paths by OS/bitness citeturn7search9
  - Registry location for LabVIEW version citeturn7search6turn7search2
  - macOS availability note citeturn7search3
  - 32 vs 64 compatibility guidance citeturn6search3turn6search11

Magic byte evidence sources (supplemental but required by prompt):
- Ryan Pacini: RSRC header + VI signature region citeturn10search7turn9search0
- LAVA community: first 16 bytes include RSRC and LVIN/LVCC citeturn9search1
- PRONOM research issue for LabVIEW VI format citeturn10search0