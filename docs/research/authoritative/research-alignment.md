...
| Webview history panel with truthful retained-pair actions, packaged docs/decision entrypoints, and a retained history-window packet | aligned with the active review surface | `src/ui/historyPanel.ts`; `src/ui/historyPanelTracker.ts`; `src/commands/openViHistoryCommand.ts`; `src/reporting/comparisonReportAction.ts`; `src/docs/bundledDocumentationAction.ts`; `src/scenarios/reviewDecisionRecordAction.ts`; `VHS-REQ-016..040`; `VHS-REQ-328..329`; `VHS-REQ-341`; `VHS-REQ-361..382`; `VHS-REQ-386..387` | sustain |
...
| Public comparison-report proof stays on one governed `runGovernedProof report-smoke` surface with canonical `LabVIEWCLI CreateComparisonReport`, while any `LVCompare` receipts remain retained internal parity evidence only | aligned in this tranche | `src/cli/runGovernedProof.ts`; `src/cli/runHarnessReportSmoke.ts`; `src/harness/harnessReportSmoke.ts`; `src/reporting/comparisonReportRuntimeExecution.ts`; `docs/product/harnesses.md`; `VHS-REQ-193..199` | sustain the canonical public surface while retaining internal parity receipts only as bounded diagnosis evidence |
...
| First-class developer dashboard concentrating multiple VI Comparison Reports across at least three commits | aligned with live dashboard proof, adaptive history-window retention, and stable latest-run discovery | `src/dashboard/comparisonReportArchive.ts`; `src/dashboard/dashboardEtaAccuracy.ts`; `src/dashboard/dashboardLatestRun.ts`; `src/dashboard/niComparisonReportParser.ts`; `src/dashboard/multiReportDashboard.ts`; `src/dashboard/multiReportDashboardAction.ts`; `src/harness/harnessDashboardSmoke.ts`; `src/cli/runHarnessDashboardSmoke.ts`; `src/ui/historyPanel.ts`; `src/commands/openViHistoryCommand.ts`; `.cache/harness-reports/HARNESS-VHS-001/dashboard-smoke.json`; `docs/architecture/adr/ADR-0009-dashboard-pair-archive-and-concentration-packet.md`; `VHS-REQ-200..215`; `VHS-REQ-221..223`; `VHS-REQ-232`; `VHS-REQ-237..238`; `VHS-REQ-268`; `VHS-REQ-295..304`; `VHS-REQ-330..340`; `VHS-REQ-347..349`; `VHS-REQ-363`; `VHS-REQ-375..376`; `VHS-REQ-380`; `VHS-REQ-388..389` | sustain the live dashboard smoke lane while validating high-history ETA behavior and future decision-support growth |
