...
| LabVIEW 2026 Q1 32/64 runtime detection and selection | aligned with live provider selection | `src/reporting/comparisonRuntimeLocator.ts`; `src/reporting/comparisonReportRuntimeExecution.ts`; `package.json`; `.cache/harness-reports/HARNESS-VHS-001/comparison-report-smoke.json`; `VHS-REQ-094..096`; `VHS-REQ-138`; `VHS-REQ-146`; `VHS-REQ-217..220`; `VHS-REQ-243..245`; `docs/architecture/adr/ADR-0006-windows64-container-isolation-for-extension-users.md` | sustain the preferred x64 container path and keep runtime-diagnostic evidence explicit on mapped logs, launch confirmation, and zero-process observations |
| First-class developer dashboard concentrating multiple VI Comparison Reports across at least three commits | partial in this tranche | `src/dashboard/comparisonReportArchive.ts`; `src/dashboard/niComparisonReportParser.ts`; `src/dashboard/multiReportDashboard.ts`; `src/dashboard/multiReportDashboardAction.ts`; `src/ui/historyPanel.ts`; `src/commands/openViHistoryCommand.ts`; `docs/architecture/adr/ADR-0009-dashboard-pair-archive-and-concentration-packet.md`; `VHS-REQ-200..215` | add dashboard smoke proof, runtime-doctor UX, and mixed-provider contribution on top of the new drill-down/provider-provenance slice |
...
3. Runtime-doctor guidance and provider troubleshooting
4. Review-scenario registry and human decision records
5. Future research refresh
