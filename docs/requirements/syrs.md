...
| VHS-SYS-REQ-007 | The system shall fail closed when an explicitly selected runtime bundle is unsupported or unavailable. | Silent fallback would make provider and runtime selection untrustworthy. | Governing product and requirements surfaces state that unsupported bundles such as Docker `x86` block compare with corrective guidance. | Documentation review and static inspection |
| VHS-SYS-REQ-008 | The system shall provide an explicit compare-preflight state instead of starting compare automatically after commit selection. | Users need to confirm the chosen commit pair and runtime facts before execution. | Governing product and requirements surfaces state that compare opens from an explicit preflight action showing selected/base commit, provider, version, and bitness. | Documentation review and static inspection |
