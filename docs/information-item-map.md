...
| Governed runner host asset pack | `scripts/gitlab-runner/` | sole author | Windows apply/bootstrap scripts, Linux apply/helper scripts, Linux service unit, or host-apply contract changes | the admitted runner startup and host-apply surfaces remain versioned in the repo and recoverable after host restart or rebuild without relying on untracked machine files |
