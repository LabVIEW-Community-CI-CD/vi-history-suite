...
| Governed runner host asset pack | `scripts/gitlab-runner/` | sole author | Windows bootstrap script, Linux helper, Linux service unit, or host-apply contract changes | the admitted runner startup surfaces remain versioned in the repo and recoverable after host restart or rebuild without relying on untracked machine files |
