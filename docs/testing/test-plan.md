...
baseline regression floors: 79% statements, 68% branches, 84% functions, and
79% lines. These floors were ratcheted toward the measured `develop` actuals at
v1.33.2, which measured 81.46% statements, 71.22% branches, 86.27% functions,
and 81.5% lines locally (the lower-running Ubuntu CI leg historically
...
| --- | --- | --- | --- | --- |
| VHS-REQ-597 | TEST-597 | .github/workflows/ci.yml; vitest.config.ts | tests/unit/branchGovernanceWorkflow.test.ts; tests/unit/requirementsDocs.test.ts | Hosted CI retains coverage artifacts, enforces evidence-backed baseline thresholds at 79% statements, 68% branches, 84% functions, and 79% lines, and runs `DoD Gate / dod` (`npm run dod:gate`) after packaging. |
