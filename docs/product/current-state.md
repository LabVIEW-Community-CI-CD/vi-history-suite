...
- active execution program: [PROGRAM-0002: Public Facade Release Kit And Host-Machine Acceptance](./execution-programs/PROGRAM-0002-public-facade-installer-and-windows-acceptance.md)
  ...
  - the latest deep-host Linux `lv_icon.vi` benchmark now fails truthfully at pair `1/138` instead of stalling indefinitely; the retained summary is `completionState=failed`, `terminalOutcome=runtime-timed-out`, the pair-failure receipt is present, and retained stderr reports `Can't load library libniDotNETCoreInterop.so`
