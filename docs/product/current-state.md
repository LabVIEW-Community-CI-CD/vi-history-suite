...
- `TRANCHE-011`: Repeatable Windows and Linux benchmark proof
  ...
  - a fresh governed repo-local exact-pair rerun now retains the active Windows-image surface explicitly: the authority repo `.cache` `comparison-report-smoke.json` now carries `runtimeLabviewIniPath=C:\Program Files (x86)\National Instruments\Shared\LabVIEW CLI\LabVIEWCLI.ini`, `runtimeLabviewTcpPort=3363`, and startup-hardening notes from the container meta line itself, while a direct probe of `nationalinstruments/labview:2026q1-windows` confirms that image exposes only x86 `LabVIEWCLI.exe` plus x64 `LabVIEW.exe`; that strengthens the pair-129 ceiling into a governed mixed-surface Windows image seam rather than a missing retained-argument seam
  ...
  - the comparable-prefix packet now also retains an explicit Windows blocker characterization for that same exact pair: the retained `labview-cli` receipt proves `runtimeExecutableBitness=x86` while the governed headless-reset `-LabVIEWPath` resolves to the x64 `LabVIEW.exe`, so the bounded Windows ceiling is now carried as `mixed-bitness-call-by-reference-seam` instead of only as a generic pair-129 runtime blocker
...
- `TRANCHE-013`: Extension execution flexibility and runtime acquisition UX
  ...
  - nine repo-side execution-policy slices are now landed: `package.json` exposes `viHistorySuite.executionMode`, canonical effective execution-request validation now governs provider choice, runtime selection retains `executionMode` as explicit provider truth, `host-only` / `docker-only` fail closed instead of silently substituting the other provider class, Windows `auto` now stays host-native on a clean validated host while routing contaminated host surfaces through Docker only when the governed provider is actually needed, Windows container selections now acquire the governed image with visible progress before packet persistence and runtime launch when the image is missing locally, the history panel now retains the latest compare-runtime summary in-panel after compare actions including rejected-provider reasons, blocked or failed compare actions now emit one concise mode-aware warning from that same retained runtime truth, the history panel now also mirrors live compare-runtime progress while the action is in flight, successful compare actions now emit one concise provider/acquisition summary from that same retained runtime truth, and reopening VI History now seeds the compare-runtime block from the last retained tracker-backed runtime truth instead of resetting to idle
  ...
  - the retained runtime doctor / packet surfaces now carry those host facts, Docker capability facts, and Windows image-acquisition state explicitly, while the long-running comparison action now surfaces governed Windows image-pull progress in the front-facing progress channel, the history panel now retains the latest compare provider/acquisition summary with next action in-panel, blocked/failed compare actions now surface one concise mode-aware warning in the transient notification channel, the history panel now mirrors governed compare-runtime progress while that action is in flight, successful compare actions now surface one concise provider/acquisition summary in the transient notification channel, and reopened panels now seed that compare-runtime block from the last retained tracker-backed runtime truth
  - this tranche still owns the remaining fuller front-facing provider/acquisition transparency beyond the landed host-fact, Docker-capability, image-acquisition, in-panel latest-runtime-summary, live panel-progress, compare-warning, compare-success, and panel-reopen persistence slices
