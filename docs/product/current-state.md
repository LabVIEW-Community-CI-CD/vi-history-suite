...
- `TRANCHE-013`: Extension execution flexibility and runtime acquisition UX
  ...
  - the retained runtime doctor / packet surfaces now carry those host facts, Docker capability facts, and Windows image-acquisition state explicitly, while the long-running comparison action now surfaces governed Windows image-pull progress in the front-facing progress channel, the history panel now retains the latest compare provider/acquisition summary with next action in-panel, blocked/failed compare actions now surface one concise mode-aware warning in the transient notification channel, and the history panel now mirrors governed compare-runtime progress while that action is in flight
  - this tranche still owns the remaining fuller front-facing provider/acquisition transparency beyond the landed host-fact, Docker-capability, image-acquisition, in-panel latest-runtime-summary, live panel-progress, and compare-warning slices
