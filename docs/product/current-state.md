...
- `TRANCHE-011`: Repeatable Windows and Linux benchmark proof
  ...
  - `VHS-REQ-442..443` are now implemented for Windows diagnosis reruns: the canonical-host Windows benchmark-image proof runner accepts a governed `--engine` override for targeted reruns while preserving the same proof-root contract, and the shared dashboard-smoke progress surface now labels the active Windows lane truthfully instead of leaking Linux wording
