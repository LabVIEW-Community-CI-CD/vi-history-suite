...
3. [Research Implementation Index](../research/authoritative/research-implementation-index.json)
4. [Research Alignment Matrix](../research/authoritative/research-alignment.md)
5. [Development Queue](./development-queue.json)
6. [Architecture Overview](../architecture/overview.md)
7. [Software Requirements Specification](../requirements/srs.md)
8. [SHIP-0001: Releasable VI History Suite](./SHIP-0001-releasable-vi-history-suite.md)
9. [wiki-authority-map.md](./wiki-authority-map.md)
10. [documentation-coherence-ledger.md](./documentation-coherence-ledger.md)
11. [wiki-seed-plan.md](./wiki-seed-plan.md)
12. [wiki-publication-ledger.md](./wiki-publication-ledger.md)
13. [wiki-publication-ledger.json](./wiki-publication-ledger.json)
14. [wiki-coverage-matrix.md](./wiki-coverage-matrix.md)
15. [wiki-coverage-matrix.json](./wiki-coverage-matrix.json)
16. [debt-retirement-contract.md](./debt-retirement-contract.md)
17. [debt-taxonomy.md](./debt-taxonomy.md)
18. [debt-ledger.md](./debt-ledger.md)
19. [debt-ledger.json](./debt-ledger.json)
20. [Documentation Package Workbench](../documentation-workbench.md)
21. [program-repo-jump.md](./program-repo-jump.md)
22. [PROGRAM-0001: Next Product Layer](./execution-programs/PROGRAM-0001-next-product-layer.md)
23. [PROGRAM-0002: Public Facade Release Kit And Host-Machine Acceptance](./execution-programs/PROGRAM-0002-public-facade-installer-and-windows-acceptance.md)
24. [PROGRAM-0003: Repeatable Benchmark Proof](./execution-programs/PROGRAM-0003-repeatable-benchmark-proof.md)
25. [PROGRAM-0004: Post-Release Sustainment And Release Cadence](./execution-programs/PROGRAM-0004-post-release-sustainment-and-release-cadence.md)
26. [post-release-sustainment-rules.md](./post-release-sustainment-rules.md)
27. [post-release-sustainment-rules.json](./post-release-sustainment-rules.json)
28. [Extension Execution Policy](./extension-execution-policy.md)
29. [PROGRAM-0005: Extension Execution Flexibility And Runtime Acquisition UX](./execution-programs/PROGRAM-0005-extension-execution-flexibility-and-runtime-acquisition-ux.md)
...
2. [CHANGELOG.md](../../CHANGELOG.md)
3. [research-implementation-index.json](../research/authoritative/research-implementation-index.json)
4. [development-queue.json](./development-queue.json)
5. [next-research-prompt.md](../research/authoritative/next-research-prompt.md)
6. [SHIP-0001: Releasable VI History Suite](./SHIP-0001-releasable-vi-history-suite.md)
7. [release-readiness-matrix.json](./release-readiness-matrix.json)
8. [blocker-ledger.json](./blocker-ledger.json)
9. [wiki-authority-map.md](./wiki-authority-map.md)
10. [documentation-coherence-ledger.md](./documentation-coherence-ledger.md)
11. [wiki-seed-plan.md](./wiki-seed-plan.md)
12. [wiki-publication-ledger.md](./wiki-publication-ledger.md)
...
28. [Extension Execution Policy](./extension-execution-policy.md)
29. [PROGRAM-0005: Extension Execution Flexibility And Runtime Acquisition UX](./execution-programs/PROGRAM-0005-extension-execution-flexibility-and-runtime-acquisition-ux.md)
30. [Fast VS Code Loop](../dev-fast-loop.md)
...
- active execution program: [PROGRAM-0002: Public Facade Release Kit And Host-Machine Acceptance](./execution-programs/PROGRAM-0002-public-facade-installer-and-windows-acceptance.md)
  ...
  - the latest retained host review at `2026-04-06T06:58:46.822Z` is `failed-human-review` with `confidence=high`: from the deterministic local fixture workspace on the updated installed bundle, the retained review note for `Tooling/deployment/VIP_Post-Uninstall Custom Action.vi` says, `Documentation is highly mediocre, it mentions nothing about the open compare buttons on the pairs, as well as many other misses you had.`
  - the Gate D blocker set is cumulative across the retained failed reviews after the runtime stall was retired:
    ...
    `2026-04-06T06:13:09.879Z` adds the broader documentation-coherence finding,
    `2026-04-06T06:18:09.177Z` adds that the history Actions column still failed to enable `Open compare` after `Open dashboard` completed,
    and `2026-04-06T06:58:46.822Z` adds that the concise installed-user guide still failed to explain pair-level `Open compare` buttons and the surrounding review workflow clearly enough for an extension user
  - the authority repo now carries a repo-side fix for that latest `Open dashboard` -> `Open compare` stale-action seam: after dashboard completion, the history panel now recomputes retained pair evidence availability and rerenders the Actions column from current retained truth
  - the authority repo now also carries repo-side fixes for the remaining additive blocker set before the next rerun: dashboard overview images are grouped into caption rows with `Block Diagram Overview` before `Front Panel Overview`, unsubmitted Review Note draft state persists across tab switches until a successful submission clears it, and the bundled installed-user docs are now a concise curated extension-user guide that strips private GitLab plus SRS/RTM authority-link exposure and explicitly explains pair-row `Generate compare` / `Open compare` / `Refresh compare` transitions, the oldest-row no-base rule, and dashboard drill-down back into pair rows
  - Gate D now remains open only on the next manual rerun of the updated installed bundle, because the cumulative blocker package has been retired in the repo again but not yet re-proven by Sergio on the canonical host after this second docs iteration
...
- `TRANCHE-011`: Repeatable Windows and Linux benchmark proof
  ...
  - while refreshed benchmark images republish, the active PROGRAM-0003 follow-on move is now a governed `runGovernedProof host-operation-matrix` lane on the canonical Windows host: it inventories the installed LabVIEWCLI operation set from `C:\Program Files (x86)\National Instruments\Shared\LabVIEW CLI\Operations`, retains separate LabVIEW 2026 x86 and x64 host cases, treats `PrintToSingleFileHtml` from the `aphill93/linuxContainerDemo` `demo` branch as the allowed repo-supplied additional operation, requires pre-run and post-run contamination inspection plus retained cleanup truth for every case, distinguishes cold CLI attach from warm headless-prelaunched LabVIEW attach, and keeps `CreateComparisonReport` gated until the simpler operations are exercised first
  - fresh governed host proof on `2026-04-06` now narrows the stale pair-135/138 blocker label materially: direct `LabVIEW 2026 x64 --headless` cold-starts cleanly on the canonical host, a cold `MassCompile -Help` attach against that same x64 surface now fails truthfully with readable help text plus `Windows host operation matrix observation window expired after 15000 ms.` while leaving no retained `LabVIEW.exe` / `LabVIEWCLI.exe` contamination, and a warm x64 headless prelaunch lets x86 `LabVIEWCLI.exe` connect on port `3363` and complete both `MassCompile -Help` and `CloseLabVIEW -Headless`; by contrast, direct `LabVIEW 2026 x86 --headless` also cold-starts cleanly but warm x86 `MassCompile -Help` still stalls after the initial CLI banner, so the old `linux-headless-recursive-load` wording is no longer precise enough by itself and the active host seam is now bounded as cold CLI attach on both surfaces plus a remaining x86 warm-attach stall
...
- `TRANCHE-012`: Post-release sustainment and release cadence
  ...
  - this tranche now owns long-tail release cadence, benchmark refresh cadence, operator-surface upkeep, and post-release control-plane maintenance after benchmark proof and execution-policy closeout both landed, while `TRANCHE-010` remains open only on Sergio Velderrain's human right-click gate
  ...
  - feature-layer extension execution flexibility and runtime-acquisition UX are now closed under `TRANCHE-013` / `PROGRAM-0005`; future feature expansion must reopen under a new explicit program instead of being absorbed into sustainment by implication
- `TRANCHE-013`: Extension execution flexibility and runtime acquisition UX
  - closed issue: [ISSUE-0410 Extension Execution Flexibility And Runtime Acquisition UX](./issues/ISSUE-0410-extension-execution-flexibility-and-runtime-acquisition-ux.md)
  ...
  - ten repo-side execution-policy slices are now landed: `package.json` exposes `viHistorySuite.executionMode`, canonical effective execution-request validation now governs provider choice, runtime selection retains `executionMode` as explicit provider truth, `host-only` / `docker-only` fail closed instead of silently substituting the other provider class, Windows `auto` now uses the governed Windows container provider whenever Docker Desktop is installed and only considers host-native execution when Docker Desktop is not installed, Windows container selections now acquire the governed image with visible progress before packet persistence and runtime launch when the image is missing locally, the history panel now retains the latest compare-runtime summary in-panel after compare actions including rejected-provider reasons, blocked or failed compare actions now emit one concise mode-aware warning from that same retained runtime truth, the history panel now also mirrors live compare-runtime progress while the action is in flight, successful compare actions now emit one concise provider/acquisition summary from that same retained runtime truth, reopening VI History now seeds the compare-runtime block from the last retained tracker-backed runtime truth instead of resetting to idle, and the history panel now renders structured compare-runtime detail rows so provider, execution mode, report/runtime status, acquisition state, rejected providers, reason, and diagnostic reason are visible as separate governed facts instead of one compressed sentence
  - execution-policy bypass is no longer allowed: future compare execution must still pass canonical execution-request validation and governed provider hard stops instead of using hidden experiment switches or alternate launch paths
  - the installed selector now derives the selected Windows `LabVIEW.ini` surface, governed VI Server TCP port, and observed host-runtime conflict facts whenever host-native execution is in play, while Docker-first Windows `auto` decides container-provider truth from governed Docker capability facts first
  ...
  - retained Windows report evidence supports that Docker-first posture on this machine family: the governed archive currently retains repeated successful `win32|windows-container|labview-cli|succeeded` report generations and no successful `win32|host-native|labview-cli|succeeded` report generations
  - the retained runtime doctor / packet surfaces now carry those host facts, Docker capability facts, and Windows image-acquisition state explicitly, while the long-running comparison action now surfaces governed Windows image-pull progress in the front-facing progress channel, the history panel now retains the latest compare provider/acquisition summary with next action in-panel, blocked/failed compare actions now surface one concise mode-aware warning in the transient notification channel, the history panel now mirrors governed compare-runtime progress while that action is in flight, successful compare actions now surface one concise provider/acquisition summary in the transient notification channel, reopened panels now seed that compare-runtime block from the last retained tracker-backed runtime truth, and the history panel now renders those same facts as separate detail rows
  - `DEBT-0006` is now retired, so the driver-seat post-release lane moves from execution-policy closeout to sustainment while `TRANCHE-010` remains open only on Sergio's human UX gate
