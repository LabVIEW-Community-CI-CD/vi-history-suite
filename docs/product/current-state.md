...
- `TRANCHE-011`: Repeatable Windows and Linux benchmark proof
  ...
  - `VHS-REQ-451` is now implemented for shared `PROGRAM-0003` admission control: the one public `runGovernedProof` surface and its `dashboard-smoke`, `decision-record`, `report-smoke`, `benchmark-linux`, and `benchmark-windows` subcommands now share one canonical proof-admission validation layer for explicit runtime override bundles, so contradictory runtime bundles are rejected before any retained benchmark or diagnosis surface can be produced
