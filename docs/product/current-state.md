...
- active execution program: [PROGRAM-0002: Public Facade Release Kit And Host-Machine Acceptance](./execution-programs/PROGRAM-0002-public-facade-installer-and-windows-acceptance.md)
  ...
  - the authority repo now carries repo-side retirement for that simplified blocker package before the next canonical rerun: overview images are grouped into caption rows with `Block Diagram Overview` before `Front Panel Overview`, unsubmitted Review Note draft state persists across tab switches until a successful submission clears it, the history panel now treats the checkbox column as the only extension-user compare control, and the bundled installed-user docs now strip private GitLab plus SRS/RTM authority-link exposure while explaining the repo-agnostic checkbox-selected two-commit workflow, the oldest-row-as-base rule, and the absence of a separate extension-user dashboard or decision-record step
  - Gate D is now narrowed to the canonical comparison-report presentation seam on `resource/plugins/lv_icon.vi`: the embedded compare-info block must render coherently on the white report background and must emphasize commit/date/author/subject instead of runtime diagnostics before the deterministic canonical rerun can close the program
