...
- active execution program: [PROGRAM-0002: Public Facade Release Kit And Host-Machine Acceptance](./execution-programs/PROGRAM-0002-public-facade-installer-and-windows-acceptance.md)
  ...
  - the latest retained host review at `2026-04-06T19:53:21.713Z` is `passed-human-review` with `confidence=high`: on the updated installed bundle, the repo-agnostic two-commit review surface at `c:\dev\labview-icon-editor` for `vi.lib/LabVIEW Icon API/API_Rectangle Dimension.ctl` recorded `Checkboxes flow works as expected so far.`
  - that retained pass narrows the open Gate D risk materially because it re-proves the simplified checkbox-selected compare flow on a real installed extension surface outside the older canonical review-family assumptions
  - the older failed reviews remain cumulative historical evidence of already-retired repo-side defects: row-button and dashboard-specific findings are now superseded by the simplified shipped scope, extension users now use checkbox selection only, need only two retained commits to use VI History, and do not use extension-user dashboard or decision-record controls
  - the authority repo now carries repo-side retirement for that simplified blocker package before the canonical rerun: overview images are grouped into caption rows with `Block Diagram Overview` before `Front Panel Overview`, unsubmitted Review Note draft state persists across tab switches until a successful submission clears it, the history panel now treats the checkbox column as the only extension-user compare control, and the bundled installed-user docs now strip private GitLab plus SRS/RTM authority-link exposure while explaining the repo-agnostic checkbox-selected two-commit workflow, the oldest-row-as-base rule, and the absence of a separate extension-user dashboard or decision-record step
  - Gate D now remains open only on a fresh manual rerun of the deterministic public-release fixture workspace and canonical VI path on the updated installed bundle, because that exact acceptance target is still not the newest retained host review even though the checkbox-flow concern has now passed elsewhere on the canonical host
