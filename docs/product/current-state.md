...
45. [Release Branch Opening v1.3.14 2026-05-08](./release-branch-opening-v1.3.14-2026-05-08.md)
...
51. [Release Branch Opening v1.3.16 2026-05-11](./release-branch-opening-v1.3.16-2026-05-11.md)
...
| Indexing and report progress uplift | implemented and active | `src/indexing/viEligibilityIndexer.ts`; `src/commands/openViHistoryCommand.ts`; `src/ui/historyPanel.ts`; `src/reporting/comparisonReportAction.ts`; `src/dashboard/multiReportDashboardAction.ts`; `tests/unit/viEligibilityIndexer.test.ts`; `tests/unit/openViHistoryCommand.test.ts`; `tests/unit/historyPanel.test.ts`; `tests/unit/comparisonReportAction.test.ts`; `VHS-REQ-086..088`; `VHS-REQ-231..236`; `VHS-REQ-305..306`; `VHS-REQ-324`; research alignment marks the former `TRANCHE-004` partial as closed by implemented notification/status-bar/webview progress, bounded report/dashboard progress, cancellation with partial-evidence retention, and trust-loss fail-closed behavior | sustain |
...
- current release-branch opening packet:
  [release-branch-opening-v1.3.16-2026-05-11.md](./release-branch-opening-v1.3.16-2026-05-11.md)
...
- current public GitHub source publication: public `main` now publishes
  ...
  source commit; public PR #69 remains retained for the `v1.3.14` source/tag
  handoff, public PR #68 remains retained for the Windows Docker Desktop
  proof-intake template and label, and public PR #67 remains the retained
  `1.3.13` public validation facade publication
...
- active execution program: [PROGRAM-0005: Extension Execution Flexibility And Runtime Acquisition UX](./execution-programs/PROGRAM-0005-extension-execution-flexibility-and-runtime-acquisition-ux.md)
  - exact `v1.3.15` now publishes the host-default Windows local
...
- `TRANCHE-004`: Add progress-surface uplift for indexing and report generation
  - closed as implemented/superseded by the later governed progress UX stack:
    indexing progress now includes notification percent, processed/total, ETA,
    cancellation, trust-loss fail-closed behavior, and a status-bar progress
    item; report/dashboard generation now retains bounded webview/notification
    progress and cancellation with partial-evidence retention under
    `ISSUE-0403` / `TRANCHE-008`
...
- installed-user and private-release docs now fail closed on scope: Windows
  ...
  - installed-user host compare now explicitly admits an already-open selected
    LabVIEW session for local `LabVIEWCLI` execution, while canonical proof and
    benchmark lanes still require a clean Windows host surface unless they pass
    the governed installed-user prelaunch admission flag
  - historical released `repo-standards-review` `v0.2.9` compliance closeout
...
- historical installed-user Docker baseline:
  - `TRANCHE-013`: Extension execution flexibility and runtime acquisition UX
    ...
    - landed the historical Docker-only compare contract and no-host-fallback
      rule
...
- trust boundary:
  ...
  - Docker is now retained as an expert/provider path and historical
    Docker-only acceptance evidence, not as the default installed extension
    setup path
