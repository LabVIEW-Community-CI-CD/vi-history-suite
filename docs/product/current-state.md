...
- `TRANCHE-013`: Extension execution flexibility and runtime acquisition UX
  ...
  - the retained runtime doctor / packet surfaces now carry those host facts, Docker capability facts, and Windows image-acquisition state explicitly, while the long-running comparison action now surfaces governed Windows image-pull progress in the front-facing progress channel and the history panel now retains the latest compare provider/acquisition summary with next action in-panel
  - this tranche still owns the remaining fuller front-facing provider/acquisition transparency beyond the landed host-fact, Docker-capability, image-acquisition, and in-panel latest-runtime-summary slices
