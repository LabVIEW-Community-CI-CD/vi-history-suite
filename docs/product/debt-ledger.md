...
| `DEBT-0006` | open | `control-plane` | high | high | `TRANCHE-013` / `ISSUE-0410` / `PROGRAM-0005` | land canonical execution-request validation, conflict-aware `auto` behavior, Windows container-capability hard stops, visible Docker pull progress, and fully front-facing provider/acquisition transparency |
...
- `DEBT-0006` is the active remaining execution-policy ambiguity after the
  first execution-mode slice landed: the installed extension now exposes
  `auto` / `host-only` / `docker-only` and rejects silent provider fallback at
  runtime selection, but canonical execution-request validation, conflict-aware
  auto behavior, Windows container-capability truth, and acquisition progress
  remain open.
