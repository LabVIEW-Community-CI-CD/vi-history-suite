...
| --- | --- | --- |
| `viHistorySuite.runtimeProvider` | `host` \| `docker` | Where comparisons run: installed LabVIEW (Host) or an NI container (Docker). Auto-selected from the detected runtime on first run (or a later launch once one is available; stays unset while none is detected), then persisted. |
