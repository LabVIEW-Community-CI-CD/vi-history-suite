...
| --- | --- | --- |
| `viHistorySuite.runtimeProvider` | `host` \| `docker` | Where comparisons run: installed LabVIEW (Host) or an NI container (Docker). Auto-selected from the detected runtime on first run (left unset if neither is detected), then persisted. |
