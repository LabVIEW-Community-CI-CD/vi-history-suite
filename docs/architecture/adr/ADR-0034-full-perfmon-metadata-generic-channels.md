...
- **Generic channels in the parser.** `parsePdhCsv` now emits, alongside the
  ...
  across a developer host and CI. This is a superset of the *counters* behind the
  named `series` (every captured counter becomes a channel; raw values — note the
  named `labviewWorkingSetMb` series is byte→MB converted
