...
describe('selectViPreviewDocument', () => {
  ...
  it('builds the interactive viewer for interactive mode with frames + nonce (VHS-REQ-659.13)', () => {
  ...
  it('falls back to the document when interactive mode has no nonce (VHS-REQ-659.13)', () => {
    const out = selectViPreviewDocument({ labviewHtml: diagramHtml(), mode: 'interactive' });
    expect(out.mode).toBe('document');
    expect(out.html).toContain("script-src 'none'");
  });

  it('falls back to the document when no block-diagram frames extract (VHS-REQ-659.13)', () => {
