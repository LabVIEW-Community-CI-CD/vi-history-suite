...
describe('release governance package', () => {
  it('retains the semver decision framework and GitFlow branch/ci topology', () => {
    ...
    expect(hostedGovernance).toContain('current `develop` package line: `1.3.11`');
    ...
    expect(hostedGovernance).toContain('chosen bump: patch-preview');
    ...
    expect(hostedGovernanceJson.openingDecision.chosenBump).toBe('patch-preview');
