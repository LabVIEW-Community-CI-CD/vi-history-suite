...
describe('diffPerStateAnalytics (VHS-REQ-707.23, #2344)', () => {
  ...
  it('fails closed on a malformed states row (missing durationMs / NaN metric) (#2344 review)', () => {
    ...
    expect(() => diffPerStateAnalytics(a, badDuration as never)).toThrow(/durationMs must be a finite, non-negative number/);
    const negDuration = structuredClone(b) as { states: { durationMs: number }[] };
    negDuration.states[0].durationMs = -1;
    expect(() => diffPerStateAnalytics(a, negDuration as never)).toThrow(/durationMs must be a finite, non-negative number/);
