...
describe('ship-control direction system', () => {
  ...
  it('keeps the repo entrypoints and release procedure aligned to the ship-control surfaces', () => {
    ...
    expect(readme).toContain('- retained exact-version releases: `v0.2.0`, `v1.0.0`, `v1.0.1`, `v1.0.2`, `v1.0.3`');
    ...
    expect(currentState).toContain('- active exact release candidate line on `develop`: `v1.0.4`');
    ...
    expect(releaseProcedure).toContain('current exact released line is `v1.0.3`');
    expect(releaseProcedure).toContain('burned exact released line is `v1.0.2`');
    expect(releaseProcedure).toContain("current published package line on `main` is `1.0.3`");
    expect(releaseProcedure).toContain("current develop package line on `develop` is `1.0.4`");
    expect(releaseProcedure).toContain('active exact release candidate line on `develop` is `v1.0.4`');
