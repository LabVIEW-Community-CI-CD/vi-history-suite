...
describe('strict semver discipline', () => {
  it('keeps the published main line aligned to the current exact release line', () => {
    ...
    expect(versionLineContract.currentDevelopPackageLine).toBe('1.3.0');
    ...
    expect(readme).toContain('- active exact release candidate line on `develop`: none; exact `v1.3.0`');
    ...
    expect(currentState).toContain('- active exact release candidate line on `develop`: none; exact `v1.3.0`');
    ...
    expect(releaseProcedure).toContain('The current develop package line on `develop` is `1.3.0`.');
    expect(releaseProcedure).toContain('The active exact release candidate line on `develop` is none; exact');
