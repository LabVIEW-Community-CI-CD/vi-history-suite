...
describe('strict semver discipline', () => {
  it('keeps the fully published exact line distinct from the current authority line', () => {
    ...
    expect((versionLineContract as any).retainedReleaseCandidateBranch).toBe('release/1.3.16');
    ...
    expect(releaseProcedure).toContain('The current authority package line on `main` is `1.3.16`.');
