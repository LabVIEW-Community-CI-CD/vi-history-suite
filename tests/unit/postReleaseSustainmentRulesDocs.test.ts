...
describe('post-release sustainment rules package', () => {
  it('retains the v1.3.7 GitHub-published / Marketplace-pending closeout contract', () => {
    ...
    expect(rules.softwareFactoryGovernance).toEqual(
      expect.objectContaining({
        status: 'github-release-published-marketplace-pending',
        activeFeatureBranch: null,
    ...
    expect(rules.softwareFactoryGovernance.recoveryBoundary).toEqual(
      expect.arrayContaining([
        ...
        'current exact GitHub act is closed and the remaining blocker is the separate VS Code Marketplace publication act for v1.3.7'
      ])
    );
