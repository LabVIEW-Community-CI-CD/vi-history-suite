...
describe('post-release control-plane coherence', () => {
  ...
  it('keeps Gate C-D status and installed-user proof truth aligned across the active post-release surfaces', () => {
    ...
    expect(currentState).toContain('- the GitHub experiment lane now publishes a dedicated headless Linux benchmark/source-experiment image');
    expect(currentState).toContain(
    ...
    expect(program).toContain('- the private GitHub experiment mirror remains distinct from both GitLab');
    ...
    expect(issue).toContain('- that GitHub experiment lane now publishes a dedicated headless Linux');
