...
const historyScript = require(path.resolve(
  ...
  summarizeHistory: (
    ...
    providerDriftTrueCount: number;
    providerDriftFalseCount: number;
    providerDriftUnknownCount: number;
...
describe('printRuntimeSettingsLiveSessionProbeHistory script', () => {
  ...
  it('summarizes retained runs and reports live-uptake-not-proven when reload-required exists', async () => {
    ...
    expect(summary.providerDriftTrueCount).toBe(1);
    expect(summary.providerDriftFalseCount).toBe(1);
    expect(summary.providerDriftUnknownCount).toBe(0);
  ...
  it('falls back to driftDetected when liveUptakeObservation is missing', async () => {
    ...
    expect(summary.providerDriftUnknownCount).toBe(1);
