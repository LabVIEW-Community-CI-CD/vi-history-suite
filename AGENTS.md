...
- Branch-protection audit JSON summaries include `duplicateCheckIds` and `duplicateCheckIdCount`; inspect them when automation depends on stable check-ID uniqueness. Markdown output adds a `Duplicate Check IDs` table only when a collision is present.
