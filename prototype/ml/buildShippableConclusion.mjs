...
const conclusion = {
  ...
  optionalProvider: {
    ...
    qwenAsGenerator: 'GPU-capture troubleshooting (qwenGpuCapture.mjs, gpu-sample-series@v1, phase-tagged per VI) settles qwen2.5:14b as a GENERATOR: DROP it, keep it only as JUDGE. (1) Its invented-number is a BACKEND-INDEPENDENT MODEL-CONTENT hazard, not an offload/VRAM artifact -- on visibletextmarker the invented "5" PERSISTS across the full feasible offload range (0% CPU-only / 43% / 70% auto) and is byte-stable at temp0/seed0; and 14b (9.95GB) cannot fully fit the 8GB card (num_gpu=99 OOMs), so forcing full GPU residency is unreachable and cannot rescue it. (2) lv-icon exposes a SECOND failure mode -- cross-load CONTENT nondeterminism at fixed 70% offload/temp0/seed0: SEQ A (lv-icon first in a fresh load) invents 4/14/18 on all 3 reps (len 302); SEQ B (visibletextmarker+mousedown first, then lv-icon) invents 4/14/18 on rep1 (len 359) then FLIPS to clean on reps 2-3 (len 346). The differing lengths prove the numbers are literally present/absent (content variance, load-history driven), not scorer rewording. So qwen is unreliable BOTH as a stable inventor (visibletextmarker) AND an unstable flipper (lv-icon) -- disqualifying as a generator, valuable as a judge; residency config cannot fix it. Evidence: prototype/gpu-capture/qwen-lv-icon-context-variance.json + the qwen2-5-14b run-root.'
  }
};
...
md.push('', '## Optional grounded provider (#2381 next thread)', '',
  ...
  `- **qwen as generator (GPU-capture verdict):** ${conclusion.optionalProvider.qwenAsGenerator}`);
