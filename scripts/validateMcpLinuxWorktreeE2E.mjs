...
const RUNTIME = { provider: 'docker', platform: 'linux', bitness: 'x64', containerImageVersion: IMAGE_VERSION };
...
async function main() {
  ...
  try {
    ...
    const health = jsonOf(await client.callTool({ name: 'get_runtime_health', arguments: { platform: 'linux', settings: { requestedProvider: 'docker', containerImageVersion: IMAGE_VERSION } } }));
