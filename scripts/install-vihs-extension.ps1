...
function Render-WindowsLauncher {
  return @(
    ...
    'if defined VI_HISTORY_SUITE_NODE_EXE ('
    '  if exist "%VI_HISTORY_SUITE_NODE_EXE%" ('
    '    set "VIHS_NODE_COMMAND=%VI_HISTORY_SUITE_NODE_EXE%"'
    '  )'
    ')'
    ...
    ')'
    'if not defined VIHS_NODE_COMMAND ('
    '  >&2 echo VI History runtime-settings CLI requires the standard VS Code runtime or a usable Node.js runtime. Install or repair VS Code, set VI_HISTORY_SUITE_NODE_EXE, or install Node.js, then rerun "VI History: Prepare Local Runtime Settings CLI" to refresh the launcher if this dependency changed.'
    '  exit /b 1'
    ')'
