...
// NEVER an automatic git side effect (it's a deliberate verb), NEVER bypasses the queue.
//
// THE POINT (post-merge bot chains): the validation gate (npm run check + mapped tests)
// runs BEFORE the PR is opened/armed, so a slice that would draw a bot finding is caught
// locally. All side effects are injected, so the orchestration unit-tests without git/gh.

/** Builds the `Prototype-Source:` provenance trailer from the source commit shas. */
function buildPrototypeSourceTrailer(shas) {
...
function parsePrototypeSourceTrailer(message) {
  const out = [];
 ...
 * the escape hatch for an entangled lineage). Reconcile MUST carry `provenance` so the
 * Prototype-Source trailer is never empty. Throws on an invalid spec.
 */
function validatePromoteSpec(spec) {
  const errors = [];
...
function promoteBranchName(spec) {
...
function buildPromoteBody(spec, trailer) {
  const parts = [];
...
/**
 * Runs the promotion. Injected deps: git.{branchExists,createBranch,applyCommits,
 * applyReconcile,push}, runGate({mode}) -> { ok, summary }, openPr(opts) -> { number,
 * url }, arm(number), log().
 *
 * CONTRACT (the whole reason this exists):
 *  - a branch-collision precheck aborts BEFORE any git mutation;
 *  - the validation gate runs AFTER the slice is applied but STRICTLY BEFORE
 *    push/openPr/arm, and is passed the slice mode (reconcile => full suite);
 ...
 */
async function runPromote(spec, deps) {
